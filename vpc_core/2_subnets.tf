...
# Subnets can't be smaller or larger than these same allowed VPC CIDR ranges.
...
resource "aws_subnet" "private_az" {
  count      = var.how_many_azs
  vpc_id     = aws_vpc.main.id
  cidr_block = cidrsubnet(var.vpc_cidr_block, var.subnet_cidr_bits, count.index + var.how_many_azs)
...
resource "aws_subnet" "secure_az" {
  count      = var.how_many_azs
  vpc_id     = aws_vpc.main.id
  cidr_block = cidrsubnet(var.vpc_cidr_block, var.subnet_cidr_bits, count.index + var.how_many_azs + var.how_many_azs)
